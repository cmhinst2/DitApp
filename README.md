...
- **포지션 커스터마이징**: 진행 내역이 없을 경우 사용자가 원하는 직무(포지션)를 선택하여 면접 시작.<img width="800" alt="캡처1" src="https://github.com/user-attachments/assets/1d34fcf0-d976-4815-b7dc-6caa932b58ff" />
- **지능형 종료 로직**:
  - **AI 자동 종료**: 질문 고갈, 부적절한 언어 사용(욕설/비방) 시 AI가 스스로 판단하여 세션 종료.<img width="800" alt="캡처" src="https://github.com/user-attachments/assets/439114c7-d5a5-4cfa-a541-16a3b1f672a7" />
  - **사용자 종료**: 사용자가 '면접 종료'를 입력하면 즉시 세션 만료.<img width="800" alt="캡처1" src="https://github.com/user-attachments/assets/cb14175c-3176-401e-8ccc-2dd49fe77d1f" />
  - **심층 피드백 제공**: 면접 종료 후 전체 대화 내용을 분석하여 보완점과 총평 제공.
- **History 관리**: 최근 일주일 내 진행된 미종료 인터뷰 이어하기 기능.<img width="800" alt="캡처" src="https://github.com/user-attachments/assets/46f339df-5c28-4027-9b14-dbf8fd86a4d7" />

- **AI 보완 가이드**: 종료된 인터뷰 데이터를 기반으로 부족한 역량을 점검받고 학습 방향을 제시받는 기능.<img width="800" alt="캡처" src="https://github.com/user-attachments/assets/9f217fd1-4e82-409c-b35d-3a86eb6e97f9" />
...
- **AI 코드리뷰** : 사용자가 작성한 코드를 AI가 리뷰해주는 기능 구축 중.
...
### [Client] 무한 스크롤 데이터 일관성 및 레이스 컨디션 해결
- **문제**: 필터 변경 시 페이지 초기화(`page=1`)와 무한 스크롤 트리거(`page++`)가 동시에 발생하여 의도치 않은 데이터 중복 요청 및 페이지 번호 오류 발생.
- **원인 분석**: 
  - React State의 비동기 업데이트 특성으로 인해 상태 변경 예약과 함수 호출 간의 시차 발생.
  - 리스트 초기화 시 발생하는 관찰자(Intersection Observer)의 오작동.
- **해결 방안**: 
  - **단일 책임 원칙**: 모든 데이터 요청을 `page` 상태 변화 감지 Effect로 일원화하여 로직의 단순화 및 예측 가능성 확보.
  - **방어적 프로그래밍**: 관찰자 내부에서 `isLoading` 및 `hasMore` 플래그를 엄격히 검사하여 불필요한 API 호출 원천 차단.
